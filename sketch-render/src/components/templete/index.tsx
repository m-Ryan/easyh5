...
export const AppContainer = () => {
  const { list, articleId, addItem, setData,setTarget, title, loading, updateStyle, deleteItem, focusElement, updatePicture } = useSelector('article');
  ...
  const onSave = useCallback(async () => {
    ...
    try {
      ...
      if (articleId) {
        await services.article.updateArticle(
            articleId,
            {
              content: zipJson,
              picture,
        ...
        const data = await services.article.addArticle(
            title,
            zipJson,
            picture
        );
    ...
    return (
      <div className={styles.container} ref={ref}>
        <div className={styles.switchTab}>
          <Radio.Group
            value={isEditor}
            onChange={val => setIsEditor(val.target.value)}
          >
  ...
  }, [isEditor, isSubmit, list, loading, onSave]);
};
