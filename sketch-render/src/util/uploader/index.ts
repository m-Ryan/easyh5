...
type UploaderEventMapHandle = { [K in keyof UploaderEventMap]: UploaderEventMap[K][] };
...
export class Uploader {
	...
	public off<K extends keyof UploaderEventMap>(event: K, fn: UploaderEventMap[K]) {
		let handles = this.handler[event] as UploaderEventMap[K][];
		this.handler[event] = handles.filter((item) => item !== fn) as UploaderEventMapHandle[K];
	}
}
